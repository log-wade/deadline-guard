...
CREATE INDEX idx_deadlines_consequence ON public.deadlines(consequence_level);-- Fix 1: Add INSERT policy for profiles table (for future manual onboarding flows)
...
FOR EACH ROW
EXECUTE FUNCTION public.check_deadline_rate_limit();-- Fix RLS policies to explicitly prevent anonymous access
